...
use super::{AuthService, Commands, OutputFormat, AudioFormat};
use crate::cli::auth::AuthManager;
use crate::bandcamp::BandcampClient;
use crate::storage::{StorageBackend, LocalStorage, WebDavStorage, SyncEngine, SyncOptions};
...
impl CommandHandler {
    pub async fn handle(command: Commands) -> Result<()> {
        match command {
            ...
            Commands::List { artist_filter, album_filter, format } => {
                Self::handle_list(artist_filter, album_filter, format).await
            }
            Commands::Scan { webdav_url, local_path, detailed } => {
                Self::handle_scan(webdav_url, local_path, detailed).await
            }
            Commands::Diff { webdav_url, local_path, missing_only } => {
                Self::handle_diff(webdav_url, local_path, missing_only).await
            }
            Commands::Sync {
                webdav_url,
                local_path,
    ...
    async fn handle_auth(service: AuthService) -> Result<()> {
        match service {
            AuthService::Bandcamp { headless, username, password, cookie, force } => {
            ...
            AuthService::Webdav { url, username, password } => {
                println!("{}", format!("Authenticating with WebDAV at {}...", url).blue());
                let (_user, _pass) = AuthManager::authenticate_webdav(
                    &url,
                    username,
                    password,
                ).await?;
    ...
    async fn handle_list(
        ...
        if let Some(album) = &album_filter {
            ...
            collection.retain(|item| {
                ...
                item.album_title.as_ref().map_or(false, |t| t.to_lowercase().contains(&album_lower))
            });
        }

        // Sort by artist then album
        collection.sort_by(|a, b| {
            a.band_name.cmp(&b.band_name)
    ...
    async fn handle_scan(webdav_url: Option<String>, local_path: Option<String>, detailed: bool) -> Result<()> {
    ...
    async fn handle_diff(webdav_url: Option<String>, local_path: Option<String>, missing_only: bool) -> Result<()> {
        let storage = Self::get_storage(webdav_url, local_path).await?;
        ...
        let client = BandcampClient::new(cookie)?;
        let bandcamp_collection = client.fetch_collection().await?;
        ...
        let missing = sync_engine.compare_collections(&bandcamp_collection, &library).await;
        ...
        if missing_only {
            ...
            println!("{}", "─".repeat(60).bright_black());
            for item in &missing {
                println!("💿 {} - {}", item.band_name.cyan(), item.item_title.bright_white());
            }
            println!();
    ...
    async fn handle_sync(
        ...
        if let Some(artist) = &artist_filter {
            let artist_lower = artist.to_lowercase();
            bandcamp_collection.retain(|item| item.band_name.to_lowercase().contains(&artist_lower));
        }
    ...
    async fn get_storage(webdav_url: Option<String>, local_path: Option<String>) -> Result<Box<dyn StorageBackend>> {
        match (webdav_url, local_path) {
            (Some(url), None) => {
                // Get WebDAV credentials from keyring or prompt
                let (username, password) = AuthManager::authenticate_webdav(
                    &url,
                    None,
                    None,
                ).await?;
        ...
        match bc_entry.get_password() {
            Ok(stored) => {
                ...
                if parts.len() >= 2 {
                    ...
                    println!("{} {} (old format, please re-auth)", 
                        "✗".yellow(),
                        "Bandcamp".yellow()
                    );
                }
