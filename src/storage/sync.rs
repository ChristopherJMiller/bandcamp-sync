...
use crate::bandcamp::{models::CollectionItem, DownloadManager};
use super::{
    StorageBackend, MusicLibraryItem, SyncOptions, DryRunResult, 
    AlbumToDownload, ConflictInfo
};
...
impl SyncEngine {
    ...
    pub async fn scan_library(&self) -> Result<Vec<MusicLibraryItem>> {
        info!("Scanning {} for existing music...", self.storage.storage_type());
        ...
        for artist_item in root_items {
            ...
            let artist_name = artist_item.path.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("")
                .to_string();
            ...
            for album_item in album_items {
                ...
                let album_name = album_item.path.file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("")
                    .to_string();
                ...
                let tracks: Vec<String> = track_items.iter()
        ...
        info!("Found {} albums in {}", library.len(), self.storage.storage_type());
        Ok(library)
    }
    ...
    pub async fn compare_collections(
        ...
        let library_set: HashSet<(String, String)> = library.iter()
            .map(|item| (item.artist.to_lowercase(), item.album.to_lowercase()))
            .collect();
        
        let missing: Vec<CollectionItem> = bandcamp.iter()
            .filter(|item| {
                let key = (
    ...
    pub async fn plan_sync(
        ...
        for item in missing_items {
            ...
            if !self.storage.exists(&album_path).await? {
                ...
                if !existing_tracks.is_empty() {
                    result.conflicts.push(ConflictInfo {
                        ...
                        reason: format!("Album directory already exists with {} files", existing_tracks.len()),
                    });
                }
            }
            ...
            if let Some(tracks) = estimated_tracks {
                ...
                for i in 1..=tracks {
                    let track_path = album_path.join(format!("track_{:02}.{}", i, self.options.audio_format.extension()));
    ...
    pub fn display_dry_run(&self, result: &DryRunResult) {
        ...
        let unknown_count = result.albums_to_download.iter()
        ...
        if unknown_count > 0 {
            if result.total_tracks > 0 {
                ...
                println!("  {} known tracks + {} albums with unknown track count", 
                    result.total_tracks, unknown_count);
                println!("  Estimated size: >{:.1} MB (partial estimate)", 
        ...
        if !result.conflicts.is_empty() {
            ...
            for conflict in &result.conflicts {
                println!("  {} - {}", conflict.path.display(), conflict.reason.yellow());
            }
            println!();
        }
        ...
        println!();
        println!("{}", "To proceed with the actual sync, run without --dry-run".cyan());
        println!();
    }
    ...
    pub async fn sync_missing(&self, missing_items: &[CollectionItem], cookie: &str) -> Result<()> {
        ...
        for (idx, item) in missing_items.iter().enumerate() {
            pb.set_message(format!("Downloading: {} - {}", item.band_name, item.item_title));
            
            // Download the album
            match self.download_and_sync_album(&download_manager, item, temp_dir.path()).await {
                Ok(_) => {
                    pb.inc(1);
                    println!(
                        "  {} {} - {}",
    ...
    async fn download_and_sync_album(
        ...
        self.storage.create_directory(&album_path).await?;
        
        // Download tracks directly from the album page (like bandcamp-dl does)
        debug!("Album: {} - {}, URL: {}", item.band_name, item.item_title, item.item_url);
        
        // Download to temp directory first
        let temp_album_dir = temp_dir.join(format!("{}-{}", artist, album));
        let downloaded_files = download_manager.download_tracks_from_album_page(&item.item_url, &temp_album_dir).await?;
        ...
        for file_name in downloaded_files {
            ...
            self.storage.write_file(&dest_path, &data).await?;
