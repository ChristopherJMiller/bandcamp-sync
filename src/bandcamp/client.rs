...
impl BandcampClient {
    ...
    pub async fn get_fan_id(&self) -> Result<i64> {
        ...
        let mut urls = vec!["https://bandcamp.com/".to_string()];
        ...
        for url in &urls {
            ...
            if url.contains("cmiller548") {
                ...
                if html.len() < 1000 {
                    debug!(
                        "Short response, might be redirect or error: {}",
        ...
        anyhow::bail!(
            "Could not find fan_id. The cookie might be expired. Try: bandcamp-sync auth bandcamp"
        )
    }
